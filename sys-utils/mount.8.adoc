...
The mount source is not canonicalized for unprivileged users, with the exception of paths in _/dev/_ where the canonicalization is necessary for symbolic links like _/dev/cdrom_ and where the result has to stay in _/dev/_. All other sources are used exactly as written in _fstab_ and symbolic links in them are rejected when the path is opened. This applies namely to the source of a *bind* or *move* operation, which is pinned by a file descriptor in the same way as the target.
...
+
This option is not supported for unprivileged (non-root) users. The temporary root-directory mount and the following bind are done by several *mount*(2) syscalls with the target given by pathname, which cannot be protected against a path component swapped in the meantime.
+
