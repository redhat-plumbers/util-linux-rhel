...
/*
 * Fetch the mount ID from kernel via statx(). Uses @fd (AT_EMPTY_PATH) if >= 0,
 * otherwise uses fs->target path.
 *
...
int mnt_fs_fetch_ids(struct libmnt_fs *fs, int fd)
{
	...
	if (fd >= 0)
		rc = mnt_id_from_fd(fd, &id);
	else
		rc = mnt_id_from_path(mnt_fs_get_target(fs), &id);
