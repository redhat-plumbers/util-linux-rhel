		      ...
		      void *data __attribute__((__unused__)))
{
	 ...
	 * ID for utab.
	 *
	 * Note that mount(2) returns no handle to the new mount, so the ID has
	 * to be read from the mount point. This is a limitation of the old
	 * API, the new mount API knows the ID from the detached tree FD before
	 * it's attached. */
